...
> 5. **Case 5**: The operation `a - b * 2` will first multiply `b` by `2` (as multiplication happens before subtraction in the order of operations), resulting in `[6, 8]`, and then subtract that from `a`, resulting in `[1 - 6, 2 - 8]` which gives `[-5, -6]`.